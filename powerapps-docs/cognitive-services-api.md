...
**注**: PowerApps でアプリをビルドするのに慣れていない場合は、この記事に進む前に、「[アプリを最初から作成する](get-started-create-from-blank.md)」を読まれることをお勧めします。
...
Text Analytics API が立ち上がり、実行中になったため、PowerApps から接続したり、API を呼び出すアプリケーションをビルドしたりできます。 これは、Text Analytics API ページのデモと同様の機能を備えた 1 画面構成のアプリです。 このビルドを始めましょう。

### <a name="create-the-app-and-add-a-connection"></a>アプリの作成と接続の追加
まず、空の携帯電話アプリを作成し、**テキスト分析**コネクタを使って接続を追加します。 これらのタスクの詳細については、「[アプリを最初から作成する](get-started-create-from-blank.md)」と「[PowerApps で接続を管理する](add-manage-connections.md)」をご覧ください。

1. PowerApps Studio で、**[ファイル]**  >  **[新規]** をクリックまたはタップして、**[空のアプリ]** の下にある **[携帯電話レイアウト]** をクリックまたはタップします。
   
    ![空のアプリ - 電話レイアウト](./media/cognitive-services-api/blank-app.png)
2. 中央のペインで、**[データに接続]** をクリックまたはタップします。
3. 右側のペインで、**[新しい接続]**  >  **[テキスト分析]** をクリックまたはタップします。
   
    ![PowerApps の [データ ソース]](./media/cognitive-services-api/data-sources.png)
...
9. 左のペインで、**[Screen1]\(画面 1\)**、[省略記号]\(**. . .**)、 **[削除]** の順番に選択します (このアプリには、この画面は不要です)。
...
2. API が返すデータは、3 つの異なる[コレクション](working-with-variables.md#create-a-collection)、つまり **languageCollect**、**sentimentCollect**、および **phrasesCollect** に格納されます。
3. ラベルの 2 つの **Text** プロパティとギャラリーの **Items** プロパティが、3 つのコレクションの内容に基づいて更新されます。
...
* 各呼び出しでは、適切なパラメーターを指定します。
  ...
  * **KeyPhrases()** と **DetectSentiment()** で、言語を "en" にハードコーディングしますが、アプリ内のロジックに基づいて、このパラメーターを渡すこともできます。 たとえば、まず言語を検出し、次に **DetectLanguage()** が返すものに基づいて、このパラメーターを設定できます。
...
アプリが完成したので、実行して動作を確認します。 次の画像では、3 つのオプションすべてが選択されており、テキストは、Text Analytics API ページの既定のテキストと同じものです。

![完成したアプリとデータ](./media/cognitive-services-api/finished-app.png)

このアプリの出力を、この記事の最初の Text Analytics API ページと比較すると、同じ結果になることが分かります。
