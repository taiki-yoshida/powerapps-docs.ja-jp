...
* 環境の Environment Admin または Environment Maker ではないが、環境内で少なくとも 1 つのアプリに対する '共同作業者’ アクセスが付与されている。 詳細については、「[share an app (アプリの共有)](share-app.md)」を参照してください。 この場合は、この環境にアプリを作成できません。 自分と共有している既存のアプリの変更だけが可能です。
